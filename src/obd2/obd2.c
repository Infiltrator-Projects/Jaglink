...
static JaglinkObd2Result obd2_write_command(
    const uint8_t *bytes, size_t byte_count, char *buffer, size_t buffer_size)
{
    ...
    if (bytes == NULL || byte_count == 0U || buffer == NULL) {
        return JAGLINK_OBD2_RESULT_INVALID_ARGUMENT;
    }
    ...
    if (buffer_size < needed) {
        ...
        return JAGLINK_OBD2_RESULT_BUFFER_TOO_SMALL;
    }
    ...
    return JAGLINK_OBD2_RESULT_OK;
}

static JaglinkObd2Result obd2_response_ready(
    const JaglinkElm327Response *response)
{
    ...
    if (response->result != JAGLINK_ELM327_RESULT_OK) {
        return JAGLINK_OBD2_RESULT_ELM_ERROR;
    }
    if (response->text[0] == '\0') {
        return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
    }
    return JAGLINK_OBD2_RESULT_OK;
}

static JaglinkObd2Result obd2_parse_hex_line(
...
static JaglinkObd2Result obd2_collect_indexed_message(
    const JaglinkElm327Response *response,
    ...
    bool *found)
{
    ...
    *found = false;
    ...
    if (result != JAGLINK_OBD2_RESULT_OK) {
        return result;
    }
...
static JaglinkObd2Result obd2_find_pid_payload(
    const JaglinkElm327Response *response,
...
next_line:
    ...
    return JAGLINK_OBD2_RESULT_UNEXPECTED_RESPONSE;
}

static JaglinkObd2Result obd2_decode_sample_data(
    uint8_t pid, const uint8_t *data, size_t length, JaglinkObd2Sample *sample)
{
    JaglinkObd2Sample decoded = {
        .pid = pid,
        .value = 0.0,
        .unit = JAGLINK_OBD2_UNIT_NONE
    };
    ...
    switch (pid) {
    case 0x04U:
        if (length < 1U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value = (double)data[0] * 100.0 / 255.0;
        decoded.unit = JAGLINK_OBD2_UNIT_PERCENT;
        break;
    case 0x05U:
        if (length < 1U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value = (double)data[0] - 40.0;
        decoded.unit = JAGLINK_OBD2_UNIT_CELSIUS;
        break;
    case 0x0bU:
        if (length < 1U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value = (double)data[0];
        decoded.unit = JAGLINK_OBD2_UNIT_KPA;
        break;
    case 0x0cU:
        if (length < 2U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value =
            (double)(((unsigned int)data[0] << 8U) | data[1]) / 4.0;
        decoded.unit = JAGLINK_OBD2_UNIT_RPM;
        break;
    case 0x0dU:
        if (length < 1U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value = (double)data[0];
        decoded.unit = JAGLINK_OBD2_UNIT_KMH;
        break;
    case 0x0fU:
        if (length < 1U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value = (double)data[0] - 40.0;
    ...
    case 0x10U:
        ...
        decoded.unit = JAGLINK_OBD2_UNIT_GRAMS_PER_SECOND;
        break;
    case 0x11U:
        if (length < 1U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value = (double)data[0] * 100.0 / 255.0;
    ...
    case 0x42U:
        ...
        decoded.unit = JAGLINK_OBD2_UNIT_VOLTS;
        break;
    case 0x46U:
        if (length < 1U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        decoded.value = (double)data[0] - 40.0;
    ...
    case 0x5eU:
        ...
        decoded.unit = JAGLINK_OBD2_UNIT_LITRES_PER_HOUR;
        break;
    case 0x78U:
        if (length < 3U) return JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE;
        if ((data[0] & 0x01U) == 0U) return JAGLINK_OBD2_RESULT_UNSUPPORTED_PID;
        decoded.value =
    ...
    case 0x7cU:
        ...
        break;
    default:
        return JAGLINK_OBD2_RESULT_UNSUPPORTED_PID;
    }
...
static uint8_t obd2_dtc_response_service(JaglinkObd2DtcKind kind)
{
    switch (kind) {
    case JAGLINK_OBD2_DTC_STORED: return 0x43U;
    case JAGLINK_OBD2_DTC_PENDING: return 0x47U;
    case JAGLINK_OBD2_DTC_PERMANENT: return 0x4aU;
    }
    return 0U;
}

static uint8_t obd2_dtc_request_service(JaglinkObd2DtcKind kind)
{
    switch (kind) {
    case JAGLINK_OBD2_DTC_STORED: return 0x03U;
    case JAGLINK_OBD2_DTC_PENDING: return 0x07U;
    case JAGLINK_OBD2_DTC_PERMANENT: return 0x0aU;
    }
    return 0U;
}

static bool obd2_dtc_exists(const JaglinkObd2DtcList *list, const char *code)
{
    size_t index;
...
static JaglinkObd2Result obd2_append_dtc_pairs(
    const uint8_t *bytes, size_t byte_count, size_t start,
    JaglinkObd2DtcKind kind, JaglinkObd2DtcList *list)
{
    size_t index;
    ...
    for (index = start; index + 1U < byte_count; index += 2U) {
        ...
        result = jaglink_obd2_decode_dtc_pair(
        ...
        if (list->count >= JAGLINK_OBD2_MAX_DTCS) {
            return JAGLINK_OBD2_RESULT_TOO_MANY_DTCS;
        }
...
{
    switch (result) {
    case JAGLINK_OBD2_RESULT_OK: return "ok";
    case JAGLINK_OBD2_RESULT_INVALID_ARGUMENT: return "invalid-argument";
    case JAGLINK_OBD2_RESULT_ELM_ERROR: return "elm-error";
    case JAGLINK_OBD2_RESULT_MALFORMED_RESPONSE: return "malformed-response";
    case JAGLINK_OBD2_RESULT_UNEXPECTED_RESPONSE: return "unexpected-response";
    case JAGLINK_OBD2_RESULT_UNSUPPORTED_PID: return "unsupported-pid";
    case JAGLINK_OBD2_RESULT_BUFFER_TOO_SMALL: return "buffer-too-small";
    case JAGLINK_OBD2_RESULT_TOO_MANY_DTCS: return "too-many-dtcs";
    case JAGLINK_OBD2_RESULT_NOT_AUTHORIZED: return "not-authorized";
    }
    return "unknown";
}
...
{
    switch (unit) {
    case JAGLINK_OBD2_UNIT_NONE: return "";
    case JAGLINK_OBD2_UNIT_PERCENT: return "%";
    case JAGLINK_OBD2_UNIT_CELSIUS: return "degC";
    case JAGLINK_OBD2_UNIT_KPA: return "kPa";
    case JAGLINK_OBD2_UNIT_RPM: return "rpm";
    case JAGLINK_OBD2_UNIT_KMH: return "km/h";
    case JAGLINK_OBD2_UNIT_GRAMS_PER_SECOND: return "g/s";
    case JAGLINK_OBD2_UNIT_VOLTS: return "V";
    case JAGLINK_OBD2_UNIT_LITRES_PER_HOUR: return "L/h";
    }
    return "";
}
    ...
    uint8_t base_pid, char *buffer, size_t buffer_size)
{
    ...
    return jaglink_obd2_build_live_pid_request(base_pid, buffer, buffer_size);
}
    ...
    char *buffer, size_t buffer_size)
{
    ...
    if (authorization == NULL || !authorization->confirmed ||
        ...
        return JAGLINK_OBD2_RESULT_NOT_AUTHORIZED;
    }
    return obd2_write_command(&service, 1U, buffer, buffer_size);
}
...
next_supported_line:
    ...
    if (base_pid <= 0xc0U) {
        continuation = jaglink_obd2_pid_set_contains(
    ...
    char vin[JAGLINK_OBD2_VIN_LENGTH + 1U])
{
    ...
    char decoded[JAGLINK_OBD2_VIN_LENGTH + 1U] = {0};
    size_t indexed_length = 0U;
    ...
    if (indexed_found) {
        ...
        for (index = 3U;
             index < indexed_length && written < JAGLINK_OBD2_VIN_LENGTH;
             ++index) {
        ...
        cursor = response->text;
        while (*cursor != '\0' && written < JAGLINK_OBD2_VIN_LENGTH) {
            const char *end = strchr(cursor, '\n');
            size_t line_length = end == NULL ? strlen(cursor)
                                             : (size_t)(end - cursor);
            uint8_t bytes[OBD2_MAX_LINE_BYTES];
            ...
            if (byte_count >= 4U && bytes[0] == 0x49U &&
                ...
                for (index = 3U;
                     index < byte_count &&
                     written < JAGLINK_OBD2_VIN_LENGTH;
                     ++index) {
    ...
    if (written != JAGLINK_OBD2_VIN_LENGTH) {
        return JAGLINK_OBD2_RESULT_UNEXPECTED_RESPONSE;
    }
    decoded[written] = '\0';
