...
{
    return service != 0U && service != JAGLINK_UDS_SERVICE_NEGATIVE_RESPONSE &&
           service <= 0xbfU;
}
...
static void uds_clear_request(JaglinkUdsClient *client)
{
...
static JaglinkUdsResult uds_client_fail(
    JaglinkUdsClient *client, JaglinkUdsResult failure)
{
    if (client != NULL) {
        client->state = JAGLINK_UDS_CLIENT_FAILED;
...
static JaglinkUdsResult uds_write_failure(
    uint8_t *buffer, size_t buffer_size, size_t *written,
    JaglinkUdsResult result)
{
    if (written != NULL) {
        *written = 0U;
    }
...
{
    switch (result) {
    case JAGLINK_UDS_RESULT_OK: return "ok";
    case JAGLINK_UDS_RESULT_COMPLETE: return "complete";
    case JAGLINK_UDS_RESULT_WAITING: return "waiting";
    case JAGLINK_UDS_RESULT_RESPONSE_PENDING: return "response-pending";
    case JAGLINK_UDS_RESULT_NEGATIVE_RESPONSE: return "negative-response";
    case JAGLINK_UDS_RESULT_INVALID_ARGUMENT: return "invalid-argument";
    case JAGLINK_UDS_RESULT_BUFFER_TOO_SMALL: return "buffer-too-small";
    case JAGLINK_UDS_RESULT_MALFORMED_PDU: return "malformed-pdu";
    case JAGLINK_UDS_RESULT_UNEXPECTED_RESPONSE: return "unexpected-response";
    case JAGLINK_UDS_RESULT_BUSY: return "busy";
    case JAGLINK_UDS_RESULT_TIMEOUT: return "timeout";
    case JAGLINK_UDS_RESULT_FAILED_STATE: return "failed-state";
    case JAGLINK_UDS_RESULT_UNSUPPORTED: return "unsupported";
    }
    return "unknown";
}
...
{
    switch (state) {
    case JAGLINK_UDS_CLIENT_IDLE: return "idle";
    case JAGLINK_UDS_CLIENT_WAITING_RESPONSE: return "waiting-response";
    case JAGLINK_UDS_CLIENT_RESPONSE_PENDING: return "response-pending";
    case JAGLINK_UDS_CLIENT_COMPLETE: return "complete";
    case JAGLINK_UDS_CLIENT_FAILED: return "failed";
    }
    return "unknown";
}
...
const char *jaglink_uds_negative_response_code_name(uint8_t code)
{
    ...
    case JAGLINK_UDS_NRC_RESPONSE_PENDING: return "response-pending";
...
{
    JaglinkUdsResponse decoded;
    uint8_t positive_service;

    if (!uds_request_service_valid(request_service) || pdu == NULL ||
        response == NULL) {
        return JAGLINK_UDS_RESULT_INVALID_ARGUMENT;
    }
    if (pdu_length == 0U) {
        return JAGLINK_UDS_RESULT_MALFORMED_PDU;
    }
    ...
    if (pdu[0] == JAGLINK_UDS_SERVICE_NEGATIVE_RESPONSE) {
        if (pdu_length != 3U || pdu[2] == 0U) {
            return JAGLINK_UDS_RESULT_MALFORMED_PDU;
        }
        if (pdu[1] != request_service) {
            return JAGLINK_UDS_RESULT_UNEXPECTED_RESPONSE;
        }
        decoded.kind = JAGLINK_UDS_RESPONSE_NEGATIVE;
        decoded.response_service = pdu[0];
        decoded.negative_response_code = pdu[2];
        *response = decoded;
        return JAGLINK_UDS_RESULT_NEGATIVE_RESPONSE;
    }
    ...
    decoded.kind = JAGLINK_UDS_RESPONSE_POSITIVE;
    decoded.response_service = pdu[0];
    decoded.data = pdu + 1U;
    decoded.data_length = pdu_length - 1U;
    *response = decoded;
    return JAGLINK_UDS_RESULT_OK;
}
    ...
    size_t *written)
{
    ...
    buffer[0] = JAGLINK_UDS_SERVICE_DIAGNOSTIC_SESSION_CONTROL;
...
{
    JaglinkUdsResponse generic;
    JaglinkUdsSessionResponse decoded;
    JaglinkUdsResult result;
    ...
    result = jaglink_uds_decode_response(
        JAGLINK_UDS_SERVICE_DIAGNOSTIC_SESSION_CONTROL,
        pdu, pdu_length, &generic);
    if (result != JAGLINK_UDS_RESULT_OK) {
        return result;
    }
    ...
    size_t *written)
{
    ...
    buffer[0] = JAGLINK_UDS_SERVICE_TESTER_PRESENT;
...
{
    JaglinkUdsResponse generic;
    JaglinkUdsResult result = jaglink_uds_decode_response(
        JAGLINK_UDS_SERVICE_TESTER_PRESENT, pdu, pdu_length, &generic);
    ...
    size_t *written)
{
    ...
    buffer[0] = JAGLINK_UDS_SERVICE_READ_DATA_BY_IDENTIFIER;
    ...
    JaglinkUdsDidRecord *record)
{
    ...
    result = jaglink_uds_decode_response(
        JAGLINK_UDS_SERVICE_READ_DATA_BY_IDENTIFIER,
        pdu, pdu_length, &generic);
    if (result != JAGLINK_UDS_RESULT_OK) {
        return result;
    }
...
JaglinkUdsResult jaglink_uds_build_defined_did_request(
    ...
    size_t *written)
{
    if (!jaglink_uds_did_definition_is_valid(definition)) {
        return uds_write_failure(buffer, buffer_size, written,
                                 JAGLINK_UDS_RESULT_INVALID_ARGUMENT);
    }
    return jaglink_uds_build_read_did_request(
...
{
    JaglinkUdsDidRecord record;
    JaglinkUdsDidValue decoded;
    JaglinkUdsResult result;

    if (!jaglink_uds_did_definition_is_valid(definition) || value == NULL) {
        return JAGLINK_UDS_RESULT_INVALID_ARGUMENT;
    }

    result = jaglink_uds_decode_read_did_response(
    ...
    const JaglinkUdsClientConfig *config)
{
    ...
    client->active_session = JAGLINK_UDS_SESSION_DEFAULT;
    client->state = JAGLINK_UDS_CLIENT_IDLE;
    client->failure = JAGLINK_UDS_RESULT_OK;
    return JAGLINK_UDS_RESULT_OK;
}
    ...
    uint64_t now_us)
{
    ...
    if (client->state == JAGLINK_UDS_CLIENT_FAILED) {
        return JAGLINK_UDS_RESULT_FAILED_STATE;
    }
    if (client->state == JAGLINK_UDS_CLIENT_WAITING_RESPONSE ||
        client->state == JAGLINK_UDS_CLIENT_RESPONSE_PENDING) {
        return JAGLINK_UDS_RESULT_BUSY;
    }
    ...
    if (service == JAGLINK_UDS_SERVICE_DIAGNOSTIC_SESSION_CONTROL) {
        uint8_t session_type;
        ...
        if ((request_pdu[1] & 0x80U) != 0U) {
            uds_clear_request(client);
            return JAGLINK_UDS_RESULT_UNSUPPORTED;
        }
    ...
    } else if (service == JAGLINK_UDS_SERVICE_TESTER_PRESENT) {
        if (request_length != 2U) {
            uds_clear_request(client);
            return JAGLINK_UDS_RESULT_MALFORMED_PDU;
        }
    ...
    } else if (service == JAGLINK_UDS_SERVICE_READ_DATA_BY_IDENTIFIER) {
    ...
    client->state = JAGLINK_UDS_CLIENT_WAITING_RESPONSE;
    client->failure = JAGLINK_UDS_RESULT_OK;
    return JAGLINK_UDS_RESULT_OK;
}
    ...
    JaglinkUdsResponse *response)
{
    ...
    if (now_us >= client->deadline_us) {
        return uds_client_fail(client, JAGLINK_UDS_RESULT_TIMEOUT);
    }

    result = jaglink_uds_decode_response(
        client->request_service, response_pdu, response_length, &decoded);
    if (result == JAGLINK_UDS_RESULT_NEGATIVE_RESPONSE) {
        *response = decoded;
        if (decoded.negative_response_code == JAGLINK_UDS_NRC_RESPONSE_PENDING) {
            client->deadline_us = infiltratr_u64_add_saturating(
                now_us, client->p2_star_timeout_us);
            client->state = JAGLINK_UDS_CLIENT_RESPONSE_PENDING;
            return JAGLINK_UDS_RESULT_RESPONSE_PENDING;
        }
        client->state = JAGLINK_UDS_CLIENT_COMPLETE;
        client->deadline_us = 0U;
        return JAGLINK_UDS_RESULT_NEGATIVE_RESPONSE;
    }
    if (result != JAGLINK_UDS_RESULT_OK) {
    ...
    if (client->request_has_subfunction &&
        client->request_service == JAGLINK_UDS_SERVICE_DIAGNOSTIC_SESSION_CONTROL) {
        JaglinkUdsSessionResponse session;
        result = jaglink_uds_decode_session_control_response(
        ...
        if (session.timing_present) {
            ...
            client->p2_star_timeout_us = infiltratr_u64_multiply_saturating(
               ...
               client->request_service == JAGLINK_UDS_SERVICE_TESTER_PRESENT) {
        result = jaglink_uds_decode_tester_present_response(
    ...
    *response = decoded;
    client->state = JAGLINK_UDS_CLIENT_COMPLETE;
    client->deadline_us = 0U;
    return JAGLINK_UDS_RESULT_COMPLETE;
}
    ...
    uint64_t now_us)
{
    ...
    if (client->state == JAGLINK_UDS_CLIENT_IDLE) {
        return JAGLINK_UDS_RESULT_OK;
    }
    if (client->state == JAGLINK_UDS_CLIENT_COMPLETE) {
        return JAGLINK_UDS_RESULT_COMPLETE;
    }
    if (now_us >= client->deadline_us) {
        return uds_client_fail(client, JAGLINK_UDS_RESULT_TIMEOUT);
    }
    return JAGLINK_UDS_RESULT_WAITING;
}
