...
static const NSTimeInterval JagLinkScanTimeoutSeconds = 12.0;
static const NSTimeInterval JagLinkConnectTimeoutSeconds = 8.0;
static const NSTimeInterval JagLinkDiscoveryTimeoutSeconds = 8.0;
static const NSTimeInterval JagLinkProbeTimeoutSeconds = 2.0;
static const NSTimeInterval JagLinkReconnectDelaySeconds = 0.75;
static const NSUInteger JagLinkWriteQueueLimit = 65536U;

@interface JagLinkBLECandidate : NSObject
...
@implementation JagLinkBLECandidate
@end

static NSComparisonResult JagLinkCompareCandidates(JagLinkBLECandidate *left,
                                                   JagLinkBLECandidate *right)
{
...
static void JagLinkSortCandidates(NSMutableArray<JagLinkBLECandidate *> *candidates)
{
    ...
    for (NSUInteger leftIndex = 0U; leftIndex < count; ++leftIndex) {
        for (NSUInteger rightIndex = leftIndex + 1U; rightIndex < count; ++rightIndex) {
            JagLinkBLECandidate *left = [candidates objectAtIndex:leftIndex];
            JagLinkBLECandidate *right = [candidates objectAtIndex:rightIndex];
            if (JagLinkCompareCandidates(left, right) == NSOrderedDescending) {
...
static BOOL JagLinkPeripheralNameLooksLikeAdapter(NSString *name)
{
...
static BOOL JagLinkRemainingBytesAreWhitespace(const uint8_t *bytes,
                                               NSUInteger start,
                                               NSUInteger length)
{
...
@interface JagLinkBLETransport () <CBCentralManagerDelegate, CBPeripheralDelegate>
@property(nonatomic, readwrite) JagLinkBLETransportState state;
...
- (void)scheduleStateTimeout:(JagLinkBLETransportState)state
                  generation:(NSUInteger)generation
                       after:(NSTimeInterval)delay
                     message:(NSString *)message
                     recover:(BOOL)recover;
- (JaglinkTransportStatus)enqueueApplicationBytes:(const uint8_t *)bytes
                                           size:(size_t)size;
- (void)setCReceiver:(JaglinkTransportReceiveFn)receiver
             context:(void *)context;
@end

@implementation JagLinkBLETransport {
    ...
    NSArray<JagLinkBLECandidate *> *_candidates;
    NSUInteger _candidateIndex;
    JagLinkBLECandidate *_probingCandidate;
    NSUInteger _probeGeneration;
    BOOL _probeSent;
    BOOL _probeParserActive;
    JaglinkElm327Parser _probeParser;
    ...
    JaglinkTransportReceiveFn _receiver;
...
- (instancetype)init
{
    self = [super init];
    if (self != nil) {
        _state = JagLinkBLETransportStateIdle;
        _statusText = @"Idle";
...
- (BOOL)isReady
{
    return self.state == JagLinkBLETransportStateReady &&
...
- (void)notifyDelegate
{
    id<JagLinkBLETransportDelegate> delegate = self.delegate;
    if (delegate != nil) {
        [delegate bleTransportDidUpdate:self];
    }
}

- (void)setState:(JagLinkBLETransportState)state status:(NSString *)status
{
    self.state = state;
...
- (void)scheduleStateTimeout:(JagLinkBLETransportState)state
                  generation:(NSUInteger)generation
                     ...
                     recover:(BOOL)recover
{
    __weak JagLinkBLETransport *weakSelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                                 (int64_t)(delay * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        JagLinkBLETransport *strongSelf = weakSelf;
...
- (void)start
{
    ...
    if (_central == nil) {
        ...
        [self setState:JagLinkBLETransportStateWaitingForBluetooth
...
- (void)disconnect
{
    ...
    [self setState:JagLinkBLETransportStateDisconnected status:@"Disconnected"];
}
...
- (void)failAndStop:(NSString *)status
{
    ...
    [self setState:JagLinkBLETransportStateFailed status:status];
}
...
- (void)recoverAfterTransientFailure:(NSString *)status
{
    ...
    [self setState:JagLinkBLETransportStateDisconnected status:status];

    __weak JagLinkBLETransport *weakSelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                                 (int64_t)(JagLinkReconnectDelaySeconds * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        JagLinkBLETransport *strongSelf = weakSelf;
        if (strongSelf == nil || !strongSelf->_startRequested ||
        ...
        if (strongSelf->_central.state == CBManagerStatePoweredOn) {
            ...
            [strongSelf setState:JagLinkBLETransportStateWaitingForBluetooth
                          status:@"Waiting for Bluetooth"];
        }
    });
}
...
- (void)beginScan
{
    ...
    [self setState:JagLinkBLETransportStateScanning
            status:@"Scanning for BLE OBD adapter"];
    [self scheduleStateTimeout:JagLinkBLETransportStateScanning
                    generation:generation
                         after:JagLinkScanTimeoutSeconds
...
- (void)centralManager:(CBCentralManager *)central
 didDiscoverPeripheral:(CBPeripheral *)peripheral
     advertisementData:(NSDictionary<NSString *, id> *)advertisementData
                  RSSI:(NSNumber *)RSSI
{
    if (!_startRequested || self.state != JagLinkBLETransportStateScanning) {
        return;
    }
    ...
    if (name.length == 0U || !JagLinkPeripheralNameLooksLikeAdapter(name)) {
        return;
    }
    ...
    [self setState:JagLinkBLETransportStateConnecting
            status:[NSString stringWithFormat:@"Connecting to %@", name]];
    [central connectPeripheral:peripheral options:nil];
    [self scheduleStateTimeout:JagLinkBLETransportStateConnecting
                    generation:generation
                         after:JagLinkConnectTimeoutSeconds
  ...
  didConnectPeripheral:(CBPeripheral *)peripheral
{
    ...
    _operationGeneration++;
    NSUInteger generation = _operationGeneration;
    [self setState:JagLinkBLETransportStateDiscovering
            status:@"Discovering adapter services"];
    [peripheral discoverServices:nil];
    [self scheduleStateTimeout:JagLinkBLETransportStateDiscovering
                    generation:generation
                         after:JagLinkDiscoveryTimeoutSeconds
...
- (void)buildAndProbeCandidates
{
    NSMutableArray<JagLinkBLECandidate *> *result = [[NSMutableArray alloc] init];
    ...
    for (CBService *service in _peripheral.services) {
        ...
        for (CBCharacteristic *notify in characteristics) {
            ...
            for (CBCharacteristic *write in characteristics) {
                ...
                JagLinkBLECandidate *candidate = [[JagLinkBLECandidate alloc] init];
    ...
    [self setState:JagLinkBLETransportStateProbing
...
- (void)probeNextCandidate
{
    ...
    _probeSent = NO;
    _probeParserActive =
        jaglink_elm327_parser_begin(&_probeParser, "ATI") == JAGLINK_ELM327_RESULT_OK;
    ...
    __weak JagLinkBLETransport *weakSelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
                                 (int64_t)(JagLinkProbeTimeoutSeconds * NSEC_PER_SEC)),
                   dispatch_get_main_queue(), ^{
        JagLinkBLETransport *strongSelf = weakSelf;
        if (strongSelf != nil && strongSelf->_startRequested &&
            strongSelf->_probeGeneration == generation &&
            strongSelf.state == JagLinkBLETransportStateProbing &&
...
- (void)sendProbeIfPossible
{
    ...
    uint8_t frame[JAGLINK_ELM327_MAX_COMMAND + 1U];
    size_t frameSize = 0U;
    if (jaglink_elm327_build_command("ATI", frame, sizeof(frame), &frameSize) !=
...
- (void)failCurrentProbe
{
    JagLinkBLECandidate *candidate = _probingCandidate;
    _probeGeneration++;
    _probingCandidate = nil;
    _probeSent = NO;
    _probeParserActive = NO;
             ...
             error:(NSError *)error
{
    ...
    if (_probingCandidate != nil &&
        ...
        size_t consumed = 0U;
        JaglinkElm327Result parseResult =
            jaglink_elm327_parser_feed(&_probeParser,
                                      value.bytes,
                                      value.length,
                                      &consumed);
        if (parseResult == JAGLINK_ELM327_RESULT_MORE_DATA) {
            return;
        }
        if (parseResult != JAGLINK_ELM327_RESULT_OK) {
            [self failCurrentProbe];
            return;
        }
        if (consumed < value.length &&
            !JagLinkRemainingBytesAreWhitespace(value.bytes,
                                               (NSUInteger)consumed,
                                               value.length)) {
            [self failCurrentProbe];
            return;
        ...
        JaglinkElm327Result finishResult =
            jaglink_elm327_parser_finish(&_probeParser, &response);
        if (finishResult != JAGLINK_ELM327_RESULT_OK ||
        ...
        [self setState:JagLinkBLETransportStateReady status:@"BLE adapter ready"];
        return;
    }

    if (self.isReady && characteristic == _selectedNotify) {
        JaglinkTransportReceiveFn receiver = NULL;
...
{
    if (!self.isReady) {
        return JAGLINK_TRANSPORT_NOT_CONNECTED;
    }
    if (bytes == NULL || size == 0U || size > (size_t)NSUIntegerMax) {
        return JAGLINK_TRANSPORT_INVALID_ARGUMENT;
    }
    if (size > (size_t)JagLinkWriteQueueLimit ||
        _writeQueue.length > JagLinkWriteQueueLimit - (NSUInteger)size) {
        return JAGLINK_TRANSPORT_BUSY;
    }
...
static JaglinkTransportStatus JagLinkCTransportConnect(void *context)
{
    JagLinkBLETransport *transport = (__bridge JagLinkBLETransport *)context;
    if (transport == nil) {
        return JAGLINK_TRANSPORT_INVALID_ARGUMENT;
    }
...
static void JagLinkCTransportDisconnect(void *context)
{
    JagLinkBLETransport *transport = (__bridge JagLinkBLETransport *)context;
    if (transport == nil) {
        return;
...
static bool JagLinkCTransportIsConnected(void *context)
{
    JagLinkBLETransport *transport = (__bridge JagLinkBLETransport *)context;
    if (transport == nil) {
        return false;
...
static JaglinkTransportStatus JagLinkCTransportWrite(void *context,
                                                   const uint8_t *data,
                                                   size_t size)
{
    JagLinkBLETransport *transport = (__bridge JagLinkBLETransport *)context;
    ...
    __block JaglinkTransportStatus status = JAGLINK_TRANSPORT_IO_ERROR;
...
static void JagLinkCTransportSetReceiver(void *context,
...
{
    JaglinkTransport result = JAGLINK_TRANSPORT_INIT;
    if (transport == nil) {
        return result;
    }

    result.context = (__bridge void *)transport;
    result.connect = JagLinkCTransportConnect;
    result.disconnect = JagLinkCTransportDisconnect;
    result.is_connected = JagLinkCTransportIsConnected;
    result.write = JagLinkCTransportWrite;
    result.set_receiver = JagLinkCTransportSetReceiver;
    return result;
}
