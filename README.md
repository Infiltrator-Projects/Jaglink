...
**Current release: 0.2.0 — X400 foundation plus read-only J2534 discovery.**

The repository contains its own Jaguar-branded public C API, portable protocol implementation, native iPhone application, native Linux application, and the Windows `jaglink-discover` OpenPort 2.0/J2534 discovery GUI. It has no MBLINK source, submodule, build-time dependency, runtime dependency, API namespace, or user-facing identity. Infiltratr Common 1.10.0 is the only directly pinned shared-library submodule.
...
`jaglink-discover` is a deliberately read-only Windows discovery tool. It supports passive 500 kbit/s CAN capture, a finite standard OBD information inventory, timestamped JSON Lines evidence, and operator annotations. Its portable safety classifier blocks writes, resets, security/authentication access, routines, DTC clearing, programming/transfer services and all unknown services by default.
...
See `docs/DISCOVER.md` for the exact capture, OBD inventory, safety and evidence behaviour.

## iPhone

The native project is `app/ios/JAGLINK.xcodeproj`. Its Jaguar-branded CoreBluetooth provider and diagnostics controller are maintained in this repository. The app provides adapter connection, generic OBD-II capability discovery, generic fault scans, live parameters, X400 network provenance and CSV export.

On macOS, `bash ./scripts/build-ios-ipa.sh` builds an unsigned physical-device IPA and its SHA-256 checksum. The **Build unsigned iPhone IPA** GitHub Actions workflow is manually runnable and uploads `JAGLINK-unsigned.ipa` plus its checksum.
...
Published releases contain two Linux forms: a normal Debian `.deb`, and `JAGLINK-<version>-linux-native.run`. The `.run` file is self-extracting source: it carries the pinned Common submodule, installs required build dependencies automatically on Debian/Ubuntu/Mint when necessary, compiles JAGLINK natively on the machine where it is executed, and installs to `/usr/local` by default. `--no-install` performs the native compile without installation and `--prefix <path>` changes the installation prefix.

## Release assets

A successful `main` release publishes the unsigned iPhone IPA and checksum, Linux `.deb`, native Linux `.run`, `jaglink-discover.exe`, and package checksums. All required build/test jobs must pass before the release job can publish those files.
