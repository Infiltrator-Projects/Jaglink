...
#define WM_JAGLINK_LOG (WM_APP + 1)
...
typedef struct app_state_ {
    ...
    jaglink_evidence_writer *evidence;
...
static void post_logf(const char *format, ...)
{
    ...
    copy = _strdup(stack);
    if (copy == NULL) return;
    if (!PostMessageA(g_app.window, WM_JAGLINK_LOG, 0U, (LPARAM)copy)) free(copy);
}
...
static void evidence_frame(const char *direction, const PASSTHRU_MSG *msg, const char *annotation)
{
    ...
    (void)jaglink_evidence_write_frame(g_app.evidence, unix_time_ns(), direction,
...
static int make_evidence_file(void)
{
    ...
    (void)snprintf(g_app.evidence_path, sizeof(g_app.evidence_path),
                   "%sJAGLINK-evidence-%04u%02u%02u-%02u%02u%02u.jsonl",
    ...
    g_app.evidence = jaglink_evidence_open(g_app.evidence_path);
...
{
    PASSTHRU_MSG tx;
    jaglink_safety_result safety;
    unsigned long count = 1UL;
    unsigned long rc;
    safety = jaglink_safety_classify(payload, payload_length);
    if (safety.decision != JAGLINK_SAFETY_ALLOW_READ_ONLY) {
        post_logf("BLOCKED service 0x%02X: %s", (unsigned int)safety.service,
                  jaglink_safety_reason_string(safety.reason));
        return 0;
    }
...
static void run_inventory(void)
{
    ...
    if (g_app.device_id == 0UL) {
        MessageBoxA(g_app.window, "Connect to the OpenPort/J2534 device first.", "JAGLINK Discover", MB_OK | MB_ICONINFORMATION);
        return;
    }
    stop_reader();
    disconnect_channel();
...
static void connect_device(void)
{
    ...
    dll[sizeof(dll) - 1U] = '\0';
    if (dll[0] == '\0') {
        MessageBoxA(g_app.window, "No J2534 FunctionLibrary DLL is selected.", "JAGLINK Discover", MB_OK | MB_ICONWARNING);
        return;
    }
    if (!make_evidence_file()) {
        MessageBoxA(g_app.window, "Cannot create the evidence JSONL file.", "JAGLINK Discover", MB_OK | MB_ICONERROR);
        return;
    }
...
{
    OPENFILENAMEA ofn;
    char path[MAX_PATH] = "JAGLINK-evidence.jsonl";
    if (g_app.evidence == NULL) {
        MessageBoxA(g_app.window, "No evidence has been recorded yet.", "JAGLINK Discover", MB_OK | MB_ICONINFORMATION);
        return;
    }
    EnterCriticalSection(&g_app.evidence_lock);
    (void)jaglink_evidence_flush(g_app.evidence);
    LeaveCriticalSection(&g_app.evidence_lock);
...
static void add_annotation(void)
{
    ...
    if (text[0] == '\0') return;
    EnterCriticalSection(&g_app.evidence_lock);
    (void)jaglink_evidence_write_annotation(g_app.evidence, unix_time_ns(), text);
    (void)jaglink_evidence_flush(g_app.evidence);
    LeaveCriticalSection(&g_app.evidence_lock);
...
static LRESULT CALLBACK window_proc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_DESTROY:
        ...
        if (g_app.evidence != NULL) {
            EnterCriticalSection(&g_app.evidence_lock);
            jaglink_evidence_close(g_app.evidence);
            g_app.evidence = NULL;
            LeaveCriticalSection(&g_app.evidence_lock);
        }
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE previous, LPSTR command_line, int show)
{
    ...
    cls.lpszClassName = "JAGLINKDiscoverWindow";
    if (RegisterClassExA(&cls) == 0U) return 1;
    window = CreateWindowExA(0U, cls.lpszClassName,
                             "jaglink-discover — OpenPort 2.0 / J2534 read-only discovery",
