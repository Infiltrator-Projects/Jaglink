...
static void elm327_session_add_unexpected(JaglinkElm327Session *session,
                                          size_t amount)
{
...
static void elm327_session_notify(JaglinkElm327Session *session)
{
...
static void elm327_session_fail(JaglinkElm327Session *session,
                                JaglinkElm327Result elm_result,
                                JaglinkTransportStatus transport_status,
                                bool needs_resync)
{
    session->status = JAGLINK_ELM327_SESSION_FAILED;
...
static void elm327_session_receive(void *context,
                                   ...
                                   size_t size)
{
    JaglinkElm327Session *session = context;
    size_t offset = 0U;
    ...
    if (session->status != JAGLINK_ELM327_SESSION_WAITING) {
    ...
    while (offset < size &&
           session->status == JAGLINK_ELM327_SESSION_WAITING) {
        size_t consumed = 0U;
        JaglinkElm327Result result = jaglink_elm327_parser_feed(
            &session->parser, data + offset, size - offset, &consumed);

        offset += consumed;
        if (result == JAGLINK_ELM327_RESULT_MORE_DATA) {
            break;
        }
        if (result != JAGLINK_ELM327_RESULT_OK) {
            elm327_session_add_unexpected(session, size - offset);
            elm327_session_fail(session, result, JAGLINK_TRANSPORT_OK, true);
            return;
        }

        result = jaglink_elm327_parser_finish(&session->parser,
                                             &session->response);
        session->elm_result = result;
        session->transport_status = JAGLINK_TRANSPORT_OK;
        session->status = JAGLINK_ELM327_SESSION_COMPLETE;
        elm327_session_add_unexpected(session, size - offset);
...
bool jaglink_elm327_session_init(JaglinkElm327Session *session,
                                ...
                                void *event_context)
{
    if (session == NULL || !jaglink_transport_is_valid(transport)) {
        return false;
    }
    ...
    session->elm_result = JAGLINK_ELM327_RESULT_OK;
    session->transport_status = JAGLINK_TRANSPORT_OK;
    session->event = event;
...
void jaglink_elm327_session_deinit(JaglinkElm327Session *session)
{
    ...
    if (jaglink_transport_is_valid(&session->transport)) {
...
{
    JaglinkTransportStatus result;

    if (session == NULL ||
        !jaglink_transport_is_valid(&session->transport)) {
    ...
    if (result == JAGLINK_TRANSPORT_OK) {
        session->status = JAGLINK_ELM327_SESSION_IDLE;
        session->elm_result = JAGLINK_ELM327_RESULT_OK;
...
void jaglink_elm327_session_disconnect(JaglinkElm327Session *session)
{
    ...
    session->transport.set_receiver(session->transport.context, NULL, NULL);
    if (session->status == JAGLINK_ELM327_SESSION_WAITING) {
        session->status = JAGLINK_ELM327_SESSION_CANCELLED;
        session->needs_resync = false;
        elm327_session_notify(session);
    } else {
        session->status = JAGLINK_ELM327_SESSION_IDLE;
    ...
    session->transport_status = JAGLINK_TRANSPORT_NOT_CONNECTED;
}
    ...
    uint64_t timeout_ms)
{
    ...
    uint64_t deadline;
    JaglinkElm327Result parser_result;
    JaglinkTransportStatus write_result;

    if (session == NULL || command == NULL || timeout_ms == 0U ||
        !jaglink_transport_is_valid(&session->transport)) {
        return JAGLINK_ELM327_SESSION_OP_INVALID_ARGUMENT;
    }
    if (session->callback_active ||
        session->status == JAGLINK_ELM327_SESSION_WAITING) {
        return JAGLINK_ELM327_SESSION_OP_BUSY;
    }
    if (session->needs_resync) {
        return JAGLINK_ELM327_SESSION_OP_NEEDS_RESYNC;
    }
    if (!session->transport.is_connected(session->transport.context)) {
        return JAGLINK_ELM327_SESSION_OP_NOT_CONNECTED;
    }
    if (!infiltratr_u64_add_checked(now_ms, timeout_ms, &deadline)) {
        return JAGLINK_ELM327_SESSION_OP_DEADLINE_OVERFLOW;
    }

    parser_result = jaglink_elm327_build_command(
        command, frame, sizeof(frame), &frame_size);
    if (parser_result != JAGLINK_ELM327_RESULT_OK) {
        session->elm_result = parser_result;
        return JAGLINK_ELM327_SESSION_OP_INVALID_ARGUMENT;
    }
    parser_result = jaglink_elm327_parser_begin(&session->parser, command);
    if (parser_result != JAGLINK_ELM327_RESULT_OK) {
        session->elm_result = parser_result;
        return JAGLINK_ELM327_SESSION_OP_INVALID_ARGUMENT;
    }
    ...
    memset(&session->response, 0, sizeof(session->response));
    session->status = JAGLINK_ELM327_SESSION_WAITING;
    session->elm_result = JAGLINK_ELM327_RESULT_MORE_DATA;
    session->transport_status = JAGLINK_TRANSPORT_OK;
    ...
    if (write_result != JAGLINK_TRANSPORT_OK) {
        elm327_session_fail(session, JAGLINK_ELM327_RESULT_MORE_DATA,
                            write_result, true);
        return JAGLINK_ELM327_SESSION_OP_TRANSPORT_ERROR;
    }

    return JAGLINK_ELM327_SESSION_OP_OK;
}
...
{
    if (session == NULL) {
        return JAGLINK_ELM327_SESSION_FAILED;
    }

    if (session->status == JAGLINK_ELM327_SESSION_WAITING &&
        now_ms >= session->deadline_ms) {
        session->status = JAGLINK_ELM327_SESSION_TIMED_OUT;
        session->transport_status = JAGLINK_TRANSPORT_TIMEOUT;
        session->needs_resync = true;
        elm327_session_notify(session);
    }
...
{
    switch (result) {
    case JAGLINK_ELM327_SESSION_OP_OK: return "ok";
    case JAGLINK_ELM327_SESSION_OP_INVALID_ARGUMENT: return "invalid-argument";
    case JAGLINK_ELM327_SESSION_OP_BUSY: return "busy";
    case JAGLINK_ELM327_SESSION_OP_NOT_CONNECTED: return "not-connected";
    case JAGLINK_ELM327_SESSION_OP_NEEDS_RESYNC: return "needs-resync";
    case JAGLINK_ELM327_SESSION_OP_DEADLINE_OVERFLOW: return "deadline-overflow";
    case JAGLINK_ELM327_SESSION_OP_TRANSPORT_ERROR: return "transport-error";
    }
    return "unknown";
}
