...
{
    if (strstr(text, needle) != NULL) return 0;
...
int main(void)
{
    ...
    writer = jaglink_evidence_open(path);
    if (writer == NULL) {
        (void)fprintf(stderr, "cannot open evidence writer\n");
        return 1;
    }

    if (jaglink_evidence_write_frame(writer, 123456789ULL, "rx", "CAN", 0x7E8U,
    ...
    if (jaglink_evidence_write_annotation(writer, 123456790ULL, "line1\nline2") != 0) {
        (void)fprintf(stderr, "cannot write evidence annotation\n");
        jaglink_evidence_close(writer);
        (void)remove(path);
        return 1;
    }
    if (jaglink_evidence_flush(writer) != 0) {
