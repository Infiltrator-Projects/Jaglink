...
final class ConnectionViewModel: NSObject, ObservableObject, JagLinkDiagnosticsControllerDelegate {
    ...
    func connect() { clearPreparedExport(); controller.start() }
    ...
    func prepareCSVExport() {
        guard let csv = controller.csvSnapshot(), let data = csv.data(using: .utf8) else { clearPreparedExport(); return }
        clearPreparedExport()
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("JAGLINK-diagnostic-evidence-\(UUID().uuidString).csv")
        do { try data.write(to: url, options: .atomic); csvExportURL = url } catch { csvExportURL = nil }
    }

    func diagnosticsControllerDidUpdate(_ controller: JagLinkDiagnosticsController) { refresh() }
