...
#define OBD_KEY(pid_value) \
    { JAGLINK_PARAMETER_PROTOCOL_OBD2, \
      JAGLINK_PARAMETER_MODULE_STANDARD_OBD2, (pid_value) }

typedef struct {
    JaglinkParameterDefinition definition;
    JaglinkObd2Unit expected_unit;
} JaglinkObd2ParameterEntry;

static const JaglinkObd2ParameterEntry jaglink_obd2_parameters[] = {
    { { OBD_KEY(0x0cU), "obd2.engine.rpm", "RPM", "Engine speed", " rpm",
        0U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_RPM },
    { { OBD_KEY(0x0dU), "obd2.vehicle.speed", "SPEED", "Vehicle speed", " km/h",
        0U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_KMH },
    { { OBD_KEY(0x0bU), "obd2.engine.map", "MAP", "Manifold pressure", " kPa",
        0U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_KPA },
    { { OBD_KEY(0x11U), "obd2.engine.throttle", "THROTTLE", "Throttle position", "%",
        0U, true, 0.0, 100.0 }, JAGLINK_OBD2_UNIT_PERCENT },
    { { OBD_KEY(0x04U), "obd2.engine.load", "LOAD", "Calculated engine load", "%",
        0U, true, 0.0, 100.0 }, JAGLINK_OBD2_UNIT_PERCENT },
    { { OBD_KEY(0x10U), "obd2.engine.maf", "MAF", "Mass air flow", " g/s",
        1U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_GRAMS_PER_SECOND },
    { { OBD_KEY(0x05U), "obd2.engine.coolant", "ECT", "Coolant temperature", " °C",
        0U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_CELSIUS },
    ...
    { { OBD_KEY(0x2cU), "obd2.diesel.egr_command", "EGR CMD", "Commanded EGR", "%",
        1U, true, 0.0, 100.0 }, JAGLINK_OBD2_UNIT_PERCENT },
    { { OBD_KEY(0x2dU), "obd2.diesel.egr_error", "EGR ERR", "EGR error", "%",
        1U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_PERCENT },
    ...
    { { OBD_KEY(0x3cU), "obd2.aftertreatment.catalyst_temp_b1s1", "CAT T", "Catalyst temperature B1S1", " °C",
        1U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_CELSIUS },
    { { OBD_KEY(0x42U), "obd2.electrical.control_module_voltage", "VOLT", "Control module voltage", " V",
        2U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_VOLTS },
    ...
    { { OBD_KEY(0x5eU), "obd2.engine.fuel_rate", "FUEL", "Engine fuel rate", " L/h",
        2U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_LITRES_PER_HOUR },
    ...
    { { OBD_KEY(0x7aU), "obd2.dpf.bank1_delta_pressure", "DPF ΔP", "DPF bank 1 differential pressure", " kPa",
        2U, false, 0.0, 0.0 }, JAGLINK_OBD2_UNIT_KPA },
...
{
    switch (protocol) {
    case JAGLINK_PARAMETER_PROTOCOL_OBD2: return "obd2";
    case JAGLINK_PARAMETER_PROTOCOL_UDS: return "uds";
    }
    return "unknown";
}
...
bool jaglink_parameter_key_is_valid(const JaglinkParameterKey *key)
{
    ...
    switch (key->protocol) {
    case JAGLINK_PARAMETER_PROTOCOL_OBD2:
        return key->module == JAGLINK_PARAMETER_MODULE_STANDARD_OBD2 &&
               key->identifier <= UINT8_MAX;
    case JAGLINK_PARAMETER_PROTOCOL_UDS:
...
{
    if (definition == NULL ||
        !jaglink_parameter_key_is_valid(&definition->key) ||
...
{
    return sample != NULL &&
           jaglink_parameter_definition_is_valid(sample->definition) &&
    ...
    size_t buffer_size)
{
    ...
    if (!jaglink_parameter_definition_is_valid(definition) ||
    ...
    size_t buffer_size)
{
    ...
    return jaglink_parameter_format_value(
...
{
    return INFILTRATR_ARRAY_LENGTH(jaglink_obd2_parameters);
}

const JaglinkParameterDefinition *jaglink_parameter_obd2_definition_at(
    size_t index)
{
    if (index >= jaglink_parameter_obd2_definition_count()) {
        return NULL;
    }
    return &jaglink_obd2_parameters[index].definition;
}
...
const JaglinkParameterDefinition *jaglink_parameter_obd2_definition(uint8_t pid)
{
    ...
    for (index = 0U; index < jaglink_parameter_obd2_definition_count(); ++index) {
        if (jaglink_obd2_parameters[index].definition.key.identifier ==
            (uint32_t)pid) {
            return &jaglink_obd2_parameters[index].definition;
        }
    }
    ...
    const char *stable_key)
{
    ...
    for (index = 0U; index < jaglink_parameter_obd2_definition_count(); ++index) {
        if (infiltratr_string_equal(
                jaglink_obd2_parameters[index].definition.stable_key,
                stable_key)) {
            return &jaglink_obd2_parameters[index].definition;
        }
    }
...
{
    size_t index;
    JaglinkParameterSample converted;
    ...
    for (index = 0U; index < jaglink_parameter_obd2_definition_count(); ++index) {
        const JaglinkObd2ParameterEntry *entry = &jaglink_obd2_parameters[index];
...
static size_t jaglink_parameter_store_find_key(
    const JaglinkParameterStore *store,
    const JaglinkParameterKey *key)
{
    size_t index;

    if (store == NULL || !jaglink_parameter_key_is_valid(key)) {
        return SIZE_MAX;
    }

    for (index = 0U; index < store->slot_count; ++index) {
        const JaglinkParameterDefinition *definition = store->slots[index].definition;
        if (definition != NULL &&
            jaglink_parameter_key_equal(&definition->key, key)) {
            return index;
        }
    }
    return SIZE_MAX;
...
static size_t jaglink_parameter_store_find_stable_key(
    const JaglinkParameterStore *store,
    const char *stable_key)
{
    size_t index;
...
{
    switch (result) {
    case JAGLINK_PARAMETER_STORE_OK: return "ok";
    case JAGLINK_PARAMETER_STORE_INVALID_ARGUMENT: return "invalid-argument";
    case JAGLINK_PARAMETER_STORE_FULL: return "full";
    case JAGLINK_PARAMETER_STORE_DUPLICATE_KEY: return "duplicate-key";
    case JAGLINK_PARAMETER_STORE_DUPLICATE_STABLE_KEY:
        return "duplicate-stable-key";
    case JAGLINK_PARAMETER_STORE_NOT_FOUND: return "not-found";
    case JAGLINK_PARAMETER_STORE_DEFINITION_MISMATCH:
...
JaglinkParameterStoreResult jaglink_parameter_store_register(
    ...
    const JaglinkParameterDefinition *definition)
{
    JaglinkParameterStoreSlot slot;

    if (store == NULL || !jaglink_parameter_definition_is_valid(definition)) {
        return JAGLINK_PARAMETER_STORE_INVALID_ARGUMENT;
    }
    if (jaglink_parameter_store_find_key(store, &definition->key) != SIZE_MAX) {
        return JAGLINK_PARAMETER_STORE_DUPLICATE_KEY;
    }
    if (jaglink_parameter_store_find_stable_key(
            store, definition->stable_key) != SIZE_MAX) {
        return JAGLINK_PARAMETER_STORE_DUPLICATE_STABLE_KEY;
    }
    if (store->slot_count >= JAGLINK_PARAMETER_STORE_DEFINITION_CAPACITY) {
        return JAGLINK_PARAMETER_STORE_FULL;
    }
    ...
    store->slot_count++;
    return JAGLINK_PARAMETER_STORE_OK;
}
...
const JaglinkParameterDefinition *jaglink_parameter_store_definition(
    ...
    const JaglinkParameterKey *key)
{
    const size_t index = jaglink_parameter_store_find_key(store, key);
...
const JaglinkParameterDefinition *jaglink_parameter_store_definition_for_stable_key(
    ...
    const char *stable_key)
{
    const size_t index = jaglink_parameter_store_find_stable_key(store, stable_key);
    return index != SIZE_MAX ? store->slots[index].definition : NULL;
}
    ...
    bool favourite)
{
    ...
    index = jaglink_parameter_store_find_key(store, key);
    if (index == SIZE_MAX) {
        return JAGLINK_PARAMETER_STORE_NOT_FOUND;
    }
    ...
    const JaglinkParameterSample *sample)
{
    ...
    JaglinkParameterSample copied;

    if (store == NULL || !jaglink_parameter_sample_is_valid(sample)) {
        return JAGLINK_PARAMETER_STORE_INVALID_ARGUMENT;
    }

    slot_index = jaglink_parameter_store_find_key(
        store, &sample->definition->key);
    if (slot_index == SIZE_MAX) {
        return JAGLINK_PARAMETER_STORE_NOT_FOUND;
    }
    if (store->slots[slot_index].definition != sample->definition) {
        return JAGLINK_PARAMETER_STORE_DEFINITION_MISMATCH;
    }
    ...
    if (store->history_count < JAGLINK_PARAMETER_STORE_HISTORY_CAPACITY) {
    ...
    JaglinkParameterSample *sample)
{
    ...
    if (store == NULL || sample == NULL ||
        !jaglink_parameter_key_is_valid(key)) {
        return false;
    }
    index = jaglink_parameter_store_find_key(store, key);
