...
#define ELM327_CAN_MAX_DECODED_PDU (JAGLINK_ELM327_MAX_RESPONSE / 2U)
...
static JaglinkElm327CanResult elm327_can_format_id_command(
    const char *prefix,
    uint32_t can_id,
    bool extended,
    char *buffer,
    size_t buffer_size)
{
    ...
    if (prefix == NULL || buffer == NULL || buffer_size == 0U ||
        ...
        return JAGLINK_ELM327_CAN_RESULT_INVALID_ARGUMENT;
    }
    ...
    if ((size_t)written + 1U > buffer_size) {
        return JAGLINK_ELM327_CAN_RESULT_BUFFER_TOO_SMALL;
    }

    memcpy(buffer, temporary, (size_t)written + 1U);
    return JAGLINK_ELM327_CAN_RESULT_OK;
}

static JaglinkElm327CanResult elm327_can_parse_hex_line(
    const char *line,
    size_t line_length,
    uint8_t *bytes,
    size_t bytes_size,
    size_t *byte_count)
{
    ...
    *byte_count = 0U;
    ...
    for (index = 0U; index < line_length; ++index) {
        ...
        if (value < 0) return JAGLINK_ELM327_CAN_RESULT_MALFORMED_RESPONSE;
        if (high < 0) {
            high = value;
            continue;
        }
        if (count >= bytes_size) {
            return JAGLINK_ELM327_CAN_RESULT_PDU_TOO_LARGE;
        }
    ...
    if (high >= 0 || count == 0U) {
        return JAGLINK_ELM327_CAN_RESULT_MALFORMED_RESPONSE;
    }
    *byte_count = count;
    return JAGLINK_ELM327_CAN_RESULT_OK;
}
...
static JaglinkElm327CanResult elm327_can_decode_indexed(
    ...
    *found = false;
    ...
    while (*cursor != '\0') {
        ...
        if (colon != SIZE_MAX) {
            ...
            if (result != JAGLINK_ELM327_CAN_RESULT_OK) return result;
            ...
            return JAGLINK_ELM327_CAN_RESULT_UNEXPECTED_RESPONSE;
        }
...
{
    switch (result) {
    case JAGLINK_ELM327_CAN_RESULT_OK: return "ok";
    case JAGLINK_ELM327_CAN_RESULT_INVALID_ARGUMENT: return "invalid-argument";
    case JAGLINK_ELM327_CAN_RESULT_BUFFER_TOO_SMALL: return "buffer-too-small";
    case JAGLINK_ELM327_CAN_RESULT_PDU_TOO_LARGE: return "pdu-too-large";
    case JAGLINK_ELM327_CAN_RESULT_ELM_ERROR: return "elm-error";
    case JAGLINK_ELM327_CAN_RESULT_MALFORMED_RESPONSE: return "malformed-response";
    case JAGLINK_ELM327_CAN_RESULT_UNEXPECTED_RESPONSE: return "unexpected-response";
    case JAGLINK_ELM327_CAN_RESULT_FAILED_STATE: return "failed-state";
    }
    return "unknown";
}
...
{
    switch (stage) {
    case JAGLINK_ELM327_CAN_STAGE_SET_HEADER: return "set-header";
    case JAGLINK_ELM327_CAN_STAGE_SET_RECEIVE_ADDRESS: return "set-receive-address";
    case JAGLINK_ELM327_CAN_STAGE_ENABLE_AUTO_FORMATTING: return "enable-auto-formatting";
    case JAGLINK_ELM327_CAN_STAGE_ENABLE_FLOW_CONTROL: return "enable-flow-control";
    case JAGLINK_ELM327_CAN_STAGE_COMPLETE: return "complete";
    case JAGLINK_ELM327_CAN_STAGE_FAILED: return "failed";
    }
    return "unknown";
}
...
JaglinkElm327CanResult jaglink_elm327_can_channel_begin(
    ...
    const JaglinkElm327CanChannelConfig *config)
{
    if (state == NULL || !jaglink_elm327_can_channel_config_is_valid(config)) {
        return JAGLINK_ELM327_CAN_RESULT_INVALID_ARGUMENT;
    }
    memset(state, 0, sizeof(*state));
    state->config = *config;
    state->stage = JAGLINK_ELM327_CAN_STAGE_SET_HEADER;
    state->failure = JAGLINK_ELM327_CAN_RESULT_OK;
    state->elm_failure = JAGLINK_ELM327_RESULT_OK;
    return JAGLINK_ELM327_CAN_RESULT_OK;
}
    ...
    size_t buffer_size)
{
    ...
    if (state == NULL || buffer == NULL || buffer_size == 0U ||
        !jaglink_elm327_can_channel_config_is_valid(&state->config)) {
        return JAGLINK_ELM327_CAN_RESULT_INVALID_ARGUMENT;
    }

    switch (state->stage) {
    case JAGLINK_ELM327_CAN_STAGE_SET_HEADER:
        return elm327_can_format_id_command(
            "ATSH", state->config.tx_can_id, state->config.extended_id,
            buffer, buffer_size);
    case JAGLINK_ELM327_CAN_STAGE_SET_RECEIVE_ADDRESS:
        return elm327_can_format_id_command(
            "ATCRA", state->config.rx_can_id, state->config.extended_id,
            buffer, buffer_size);
    case JAGLINK_ELM327_CAN_STAGE_ENABLE_AUTO_FORMATTING:
        fixed = "ATCAF1";
        break;
    case JAGLINK_ELM327_CAN_STAGE_ENABLE_FLOW_CONTROL:
        fixed = "ATCFC1";
        break;
    case JAGLINK_ELM327_CAN_STAGE_COMPLETE:
    case JAGLINK_ELM327_CAN_STAGE_FAILED:
        return JAGLINK_ELM327_CAN_RESULT_FAILED_STATE;
    }
    ...
    const JaglinkElm327Response *response)
{
    if (state == NULL || response == NULL ||
        state->stage == JAGLINK_ELM327_CAN_STAGE_COMPLETE ||
        state->stage == JAGLINK_ELM327_CAN_STAGE_FAILED) {
        return JAGLINK_ELM327_CAN_RESULT_INVALID_ARGUMENT;
    }
    if (response->result != JAGLINK_ELM327_RESULT_OK) {
        state->failure = JAGLINK_ELM327_CAN_RESULT_ELM_ERROR;
        state->elm_failure = response->result;
        state->stage = JAGLINK_ELM327_CAN_STAGE_FAILED;
        return state->failure;
    }
    if (!response->ok_seen) {
        state->failure = JAGLINK_ELM327_CAN_RESULT_MALFORMED_RESPONSE;
        state->elm_failure = JAGLINK_ELM327_RESULT_MALFORMED_RESPONSE;
        state->stage = JAGLINK_ELM327_CAN_STAGE_FAILED;
        return state->failure;
    }

    state->stage = (JaglinkElm327CanStage)((unsigned int)state->stage + 1U);
    return JAGLINK_ELM327_CAN_RESULT_OK;
}
...
JaglinkElm327CanResult jaglink_elm327_can_build_pdu_command(
    ...
    size_t *written)
{
    char temporary[JAGLINK_ELM327_MAX_COMMAND];
    size_t index;
    ...
    buffer[0] = '\0';
    if (pdu_length > JAGLINK_ELM327_CAN_MAX_REQUEST_PDU) {
        return JAGLINK_ELM327_CAN_RESULT_PDU_TOO_LARGE;
    }
    ...
    size_t *pdu_length)
{
    ...
    if (response->result != JAGLINK_ELM327_RESULT_OK) {
        return JAGLINK_ELM327_CAN_RESULT_ELM_ERROR;
    }
